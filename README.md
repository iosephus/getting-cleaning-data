...
The steps performed by the script are described in the following sections.
...
retrieved and files are then classified, according to filename patterns, into
...
files, files with zero size, or files not belonging to these three categories 
are ignored.

In this step one list of files is created for each category, to be able to load 
the files to data frames at with a single line of code using the `lapply` 
function. The loading list would look like the following:

```
load.list.train <- list(
...
The advantage of this method is that as the amount of files grow there is no 
need to remember all the names in the loading stage and the code is reusable for
other similar files, because the names are determined automatically. It also
makes the data merging very easy.

Merge the training and test set
-------------------------------

The test and training data are stored in similar sets of files, whose names 
only differ in a suffix before the extension ("_train.txt", "_test.txt"). 
Thanks to this, our loading list for both cases have the same names for the 
data frames and can be merged easily with:
...
merging is also possible because the training and test data files have the same
...
to avoid messing the order of the time windows. 
...
There is an issue here with the columns names: The list of names appearing 
...
duplicated the column labelling for the data columns is done with:
...
Which also stored the number of windows for each subject/activity combination in
the data frame `count` for later use.
...
128-columns rows in these files contain some duplicate data. This is done in the
...
This is done by applying the function `aggregate` to the original tidy data set 
